...
**Timekeeping** (added 2026-06, `beacon_v2`-native) is wired end-to-end: NFC tap on a Raspberry Pi at the office OR a web/mobile punch button in Beacon → `timeclock-punch` Edge Function → `time_punches` insert → DB trigger `fn_punch_reconcile` TOGGLES the open interval (closes it + opens a fresh one of the opposite presence) + classifies via time-of-day rules → async `timeclock-classify` Edge Function correlates against the per-user Outlook calendar mirror in `user_calendar_events` and fires "tag your meeting" alerts for untagged out-of-office gaps. **As of `20260605120000_timekeeping_out_intervals.sql`, punched-OUT periods are first-class `time_intervals` rows (`is_out=true`) and punch DIRECTION drives counting + color: IN = at desk = counts/green; OUT = physically out = never counts/red, the category being only a label. `fn_classify_interval` rules apply to OUT intervals only; `fn_recompute_day` counts IN minutes only; frontend colors via `intervalTone(iv)`, not `TK_CATEGORY_TONE`. Requires redeploy of `timeclock-punch` + `timeclock-classify`.** As of `20260606120000_correction_add_interval.sql`, the `add_interval` correction kind backs the Worked/Away toggle on `CorrectionModal`'s "Add New Time" block: one atomic correction (`{start_at,end_at,is_out,category,note}`) that `timeclock-admin` resolves by inserting both boundary punches, rebuilding the day, then stamping the carved interval's `category` with `category_source='user'` — requires redeploy of `timeclock-admin`. `20260606120100_rebuild_dedup_punches.sql` hardens `fn_rebuild_user_day` against duplicate-instant punches (`select distinct on (punched_at)` so a zero-length interval can't flip parity); `20260606120200_punch_reconcile_backdated_guard.sql` fixes the `time_intervals_check` (`end_at >= start_at`) violation seen when approving an `add_interval` / back-dated `add_punch` — `fn_punch_reconcile` now re-derives the whole day via `fn_rebuild_user_day` whenever the incoming punch is not the user's latest (back-dated), instead of incrementally closing the open interval at a time before its start; DB-only, no redeploy. Weekly approvals via `timesheet_weeks`; approved weeks lock through a BEFORE trigger that respects an admin-set session GUC bypass (`beacon_v2.timekeeping_bypass_lock=on`). Personal **Timesheet** tab visible to all users; admin **Time Admin** tab carries Team Day + Approvals + NFC enrollment + Settings. **Admin Day editor (2026-06):** `UserDayModal` is a full editing canvas (`EditableDayTimeline`) — drag a block to move it, drag its edges to resize, drag empty track to add a block, click to retag/comment/delete — plus a `ReviewRail` atop the Team view that surfaces pending corrections + week submissions with inline Approve/Reject/Open-day. All admin edits apply IMMEDIATELY with **no Edge Function redeploy**: they write `time_punches`/`time_intervals` directly with the admin's JWT (`tk_*_admin_write` RLS + the lock guard's `is_current_user_admin()` path) then re-derive via the `authenticated`-granted `fn_rebuild_user_day`/`fn_recompute_day` RPCs. Data layer in `data.js`: `ctWallMinToISO`, `adminEditPunches`, `adminAddInterval`, `adminDeleteInterval`, `adminReclassifyInterval`, `loadWeekLock`, `loadCorrectionsForDay`. Adding a block depends on migration `20260606120200`. Activation: apply migrations `20260601120000`–`120500`, redeploy `outlook-sync` (now syncs every user via pass B), `send-alert` (migrated to `beacon_v2` + new `timesheet` render branch), and deploy `timeclock-punch`/`-classify`/`-admin` (new). Set Supabase secret `TIMECLOCK_DEVICE_KEY` (random 32-byte hex); add GH repo secrets `TIMECLOCK_CLASSIFY_URL`/`AUTH`; flip `app_settings.tk_enabled=true`; re-run `scripts/setup_outlook_rbac.ps1` for the broader scope; provision the Pi (`pi/deploy/provision.sh` — `MODE=kiosk` for the 7" touchscreen, `MODE=headless` for the OLED daemon); enroll per-user NFC fobs via Time Admin → NFC enrollment. **Pi front-end: `pi/kiosk.py` is a full-screen PyQt5 app for the official 7" touchscreen** — on tap it shows the name + IN/OUT and a tap-to-pick category, then POSTs a device-authed `action:'tag'` request to `timeclock-punch` (which now also returns `open_interval_id`) to label the just-opened interval (`category_source='user'`, recent-interval guard). `pi/timeclock.py` (headless OLED daemon) is the alternative; shared `Config`/`reader_target`/`post_punch`/`post_tag` live there. The device secret never leaves the Pi.
...
- **Team Calendar tab** (`frontend/src/team-calendar.jsx`, react-big-calendar) — **read-only** consolidated overlay of every colleague's Outlook busy-time, color-coded per person; replaces Outlook's stacked-calendar UX. Reads `beacon_v2.user_calendar_events` (the per-user mirror from `outlook-sync` Pass B) cross-user via the `tk_calevents_team_select` RLS policy (migration `20260528120000_team_calendar_visibility.sql`), which only exposes `sensitivity` normal/null + non-cancelled rows — private/confidential/cancelled events never reach the client. No authoring affordances at all (no create/edit/drag/rename); clicking opens a read-only metadata popover. Distinct from the editable Events & Other calendar (`events-calendar.jsx`, the shared `beacon@msmmeng.com` BD calendar). Related newer migrations: `20260529120000_protect_calendar_event_fields.sql` (BEFORE UPDATE trigger preserves non-empty `subject`/`location`/`attendees` against Graph's sparse cancelled-occurrence delta payloads — was wiping titles), `20260603120000_leads_type.sql` (`leads.type` ∈ Engineering/AI), `20260604120000_events_auto_type.sql` (trigger auto-derives `events.type` from the title when NULL, so Outlook-synced events get a sensible category without the Edge Function knowing the rules).
